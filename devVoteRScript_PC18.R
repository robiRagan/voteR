...
# install_github("robiRagan/voteR2") # This installs the voteR package used to runs the simulations
...
# Current Options are "uniform", "narrowNormal", "wideNormal" and "bimodal"
    ...
    # 
        # numVoters (N): Number of voters partisipating in the election.
            ...
            numVotersSweep <- seq(minNumVoters, maxNumVoters, by = 100)
    ...
    #####################################################
        ...
        names(distanceToMarginalMedians) <- newCompetitorPositions$competitorID
         ...
         for(tloop in tloopStart:numTimePeriodsGlobal){        ###### BEGIN tloop number of total loops to run with a given set of parameters #####
             cat("*****************Distn =",distributionTypeForRound,"N =",numVoters,"M =",numCompetitors,"S =",numSeats,"a =",abstentionThreshold,"nu =",numVotesPerVoter,"c =",cumulationParameter,"f =",electoralFormula,"ROUND =",rloop,"TIME =",tloop,"\n") 
